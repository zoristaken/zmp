...
use zmp_lib::{song_filter::SongFilterService, sqlite::SqliteImpl};
...
async fn integration_add_inserts_song_filter() {
    ...
    service.add(&pool, 3, 1).await.unwrap();

    let items = service.get_all(&pool).await.unwrap();
...
async fn integration_add_multiple_inserts_all_song_filters() {
    ...
    service
        .add_multiple(&pool, sample_song_filters())
        .await
        .unwrap();
...
async fn integration_get_by_id_returns_matching_row() {
    ...
    let item = service.get_by_id(&pool, 3).await.unwrap();
    assert_eq!(item.id, 3);
...
async fn integration_get_by_filter_returns_matching_rows() {
    ...
    let items = service.get_by_filter(&pool, 1).await.unwrap();
...
async fn integration_get_by_filter_returns_empty_when_missing() {
    ...
    let items = service.get_by_filter(&pool, 999).await.unwrap();
...
async fn integration_get_by_song_returns_matching_rows() {
    ...
    let items = service.get_by_song(&pool, 1).await.unwrap();
    assert_eq!(items.len(), 2);
    assert_eq!(items[0].id, 1);
...
async fn integration_get_by_song_returns_empty_when_missing() {
    ...
    let items = service.get_by_song(&pool, 999).await.unwrap();
    assert!(items.is_empty());
}
