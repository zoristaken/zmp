...
migration_name = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_create_songs.sql"
...
with open(filename, mode='w', encoding='utf-8') as f:
    ...
    for _ in range(num_rows):
        ...
        f.write(f"INSERT OR IGNORE INTO song (title, artist, release_year, album, remix, search_blob, file_path) VALUES ('{title_esc}', '{artist_esc}', {release_year}, '{album_esc}', '{remix_esc}', '{search_blob_esc}', '{file_path_esc}');\n")
